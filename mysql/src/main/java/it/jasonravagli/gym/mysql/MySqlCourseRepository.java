...
public class MySqlCourseRepository implements CourseRepository {
	...
	private static final String QUERY_FIND_ALL = "SELECT BIN_TO_UUID(id) as uuid, name FROM " + TABLE_COURSES
			+ " ORDER BY name";
	private static final String QUERY_FIND_SUBS = "SELECT BIN_TO_UUID(id) as uuid_member, surname as surname_member, name as name_member, date_of_birth FROM "
			+ TABLE_SUBS + " as s INNER JOIN " + TABLE_MEMBERS
			+ " as m ON s.id_member = m.id WHERE BIN_TO_UUID(s.id_course) = ? ORDER BY surname";
	private static final String QUERY_FIND_BY_ID = "SELECT BIN_TO_UUID(c.id) as uuid, c.name as name, BIN_TO_UUID(m.id) as uuid_member, "
	...
	private static final String QUERY_INSERT_COURSE = "INSERT INTO " + TABLE_COURSES
			+ "(id, name) VALUES(UUID_TO_BIN(?), ?)";
	private static final String QUERY_INSERT_SUB = "INSERT INTO " + TABLE_SUBS
			+ "(id_course, id_member) VALUES(UUID_TO_BIN(?), UUID_TO_BIN(?))";
	private static final String QUERY_UPDATE_COURSE = "UPDATE " + TABLE_COURSES + " SET name=? WHERE BIN_TO_UUID(id)=?";
	private static final String QUERY_DELETE_COURSE = "DELETE FROM " + TABLE_COURSES + " WHERE BIN_TO_UUID(id) = ?";
	private static final String QUERY_DELETE_SUBS = "DELETE FROM " + TABLE_SUBS + " WHERE BIN_TO_UUID(id_course) = ?";
	...
	public void save(Course course) throws SQLException {
		PreparedStatement statInsertCourse = connection.prepareStatement(QUERY_INSERT_COURSE);
	...
	public void update(Course updatedCourse) throws SQLException {
		PreparedStatement statUpdateCourse = connection.prepareStatement(QUERY_UPDATE_COURSE);
